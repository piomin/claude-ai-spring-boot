...
- Minimize the amount of code generated. Prefer modern Java features that remove boilerplate (records for DTOs/value types, explicit constructors for injection).
...
- Lombok is allowed when a project uses it. Default to modern Java (records, explicit accessors); reach for Lombok to cut entity/service boilerplate, following the dos/don'ts in the `spring-boot` skill — notably never `@Data`/`@EqualsAndHashCode`/`@ToString` on JPA/Mongo entities. Be consistent within a module.
